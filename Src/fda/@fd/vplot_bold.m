function Hline = vplot_bold(fdobj, unit, Lfdobj, matplt, href, nx)
...
if nargin < 2 || isempty(unit),   unit = 0;             end
...
if ndim == 2
    ...
    if matplt
        ...
        if href && (frng(1) <= 0 && frng(2) >= 0)
            if nargout > 0
                Hline = plot(fdmat, x, '-','LineWidth',3);
            else
                plot(fdmat, x);
            end
        else
            if nargout > 0
                ...
                plot (fdmat, x, '-','LineWidth',3)
            end
        end
        xlabel(['\fontsize{12} ',fdnames{2}]);
        
        if unit==0
            set(gca,'Ydir','reverse')
            ylabel(['\fontsize{12} ','hPa']);
        else
            ylabel(['\fontsize{12} ','km']);
        end
        if frng(2) > frng(1)
            %axis([frng(1), frng(2), x(1), x(nx)]);
        end
        title(['\fontsize{12} ',fdnames{3}]);
    else
