...
ground.X_p{1} = ones(n1,1,1,2);
ground.X_p_name{1} = {'constant','constant'};

ground.X_p{2} = ones(n2,1,1,2);
ground.X_p_name{2} = {'constant','constant'};
...
obj_stem_par.alpha_p = [0.3 0.6;
                        0.6 0.3]';
obj_stem_par.theta_p = [10 50];
obj_stem_par.v_p(:,:,1) = [1 0.6; 0.6 1];
%obj_stem_par.v_p(:,:,2) = [1 0.6; 0.6 1];
...
obj_stem_sim=stem_sim(obj_stem_model);
obj_stem_sim.simulate;
