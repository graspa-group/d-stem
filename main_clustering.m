...
if flag_simulatedata
    RandStream.setGlobalStream(RandStream('mt19937ar','seed',2222));
    %Data simulation
    phase_step=20; %degree
    t=1:365;
    if 1
    ...
    if flag_lakedata
        if flag_singlelake
            ...
            load C:\Francesco\Dropbox\Universita\Ricerca\Visiting\2012_Glasgow\ricerca\arclake\lake_average\lakes_data
            load C:\Francesco\Dropbox\Universita\Ricerca\Visiting\2012_Glasgow\ricerca\arclake\lake_average\lakes_covariates.mat
            idx=[];
            ...
            shape = shaperead('C:\Francesco\Dropbox\Universita\Ricerca\Visiting\2012_Glasgow\ricerca\arclake\lake_average\CNTR_BN_03M_2010');
            temp=data.day;
            if flag_remove_zero
                temp(temp<=273.16)=NaN;
            end
            load C:\Francesco\Dropbox\Universita\Ricerca\Visiting\2012_Glasgow\ricerca\arclake\lake_average\residual_timeseries
            %sd_g.Y{1}=double(temp);
            %sd_g.Y_name={'temperature'};
            %st_grid=stem_grid(data.coordinates_day,'deg','sparse','point');
            load abstract
            sd_g.Y{1}=abstract;
            sd_g.Y_name={'frequenze'};
            st_grid=stem_grid(rand(2345,2),'deg','sparse','point');
        end
    else
        if flag_no2
...
st_par.sigma_eta=diag(repmat(10,n_clusters,1));
